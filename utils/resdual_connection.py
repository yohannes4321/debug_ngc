...
class ResidualConnectionComponent(JaxComponent):
    ...
    def __init__(self, name, input_shape, output_shape, **kwargs):
        ...
        self.inputs_1 = Compartment(jnp.zeros(input_shape))
        self.inputs_2 = Compartment(jnp.zeros(input_shape))
        self.outputs = Compartment(jnp.zeros(output_shape))
        
    
    @transition(output_compartments=["outputs"])
    @staticmethod
    def advance_state(input_1,inputs_2):
        result=input_1+inputs_2
        return result
