...
class Attention:
    ...
    def __init__(self, dkey, n_embed, seq_len, batch_size, n_heads, dropout_rate, eta, optim_type, wub, wlb, prefix,tau_m,act_fx, **kwargs):
        ...
        self.z_q = RateCell(
                    f"{prefix}z_q", n_units=n_embed, tau_m=tau_m, act_fx=act_fx, prior=("gaussian", 0.),
                    integration_type="euler"
                )
        self.z_k = RateCell(
                    f"{prefix}z_k", n_units=n_embed, tau_m=tau_m, act_fx=act_fx, prior=("gaussian", 0.),
                    integration_type="euler"
                )
        self.z_v = RateCell(
                    f"{prefix}z_v", n_units=n_embed, tau_m=tau_m, act_fx=act_fx, prior=("gaussian", 0.),
                    integration_type="euler"
                )
        self.e_q = ErrorCell(f"{prefix}e_q", n_units=n_embed, 
                                  batch_size=batch_size * seq_len)
        self.e_k = ErrorCell(f"{prefix}e_k", n_units=n_embed, 
                                  batch_size=batch_size * seq_len)
        self.e_v = ErrorCell(f"{prefix}e_v", n_units=n_embed, 
                                  batch_size=batch_size * seq_len)
        ...
        self.E_qkv = StaticSynapse(f"{prefix}E_qkv", shape=(n_embed, n_embed),
                        weight_init=dist.uniform(amin=wlb, amax=wub), key=subkeys[5])