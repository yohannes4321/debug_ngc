...
class NGCTransformer:
            ...
            with Context("Circuit") as self.circuit:
                ...
                for i in range(n_layers):
                    ...
                    block=Block(dkey=subkey, block_id= i, n_embed=n_embed, seq_len=seq_len,
                                batch_size=batch_size, vocab_size=vocab_size, n_heads=n_heads, dropout_rate=dropout_rate, eta=eta, optim_type=optim_type, wub=wub, wlb=wlb,tau_m=tau_m,act_fx=act_fx)
                ...
                for blocks in range(n_layers):
                    ...
                    block.attention.e_q.mu  << block.attention.W_q.outputs
                    block.attention.e_q.target  << block.attention.z_q.z

                    block.attention.e_k.mu  << block.attention.W_k.outputs
                    block.attention.e_k.target  << block.attention.z_k.z


                    block.attention.e_v.mu << block.attention.W_v.outputs
                    block.attention.e_v.target  << block.attention.z_v.z
                    
                    block.reshape_2d_to_3d_q.inputs  << block.attention.z_q.zF
                    block.reshape_2d_to_3d_k.inputs  << block.attention.z_k.zF
                    block.reshape_2d_to_3d_v.inputs  << block.attention.z_v.zF
                    ...
                    # code update in my method 
                    block.attention.E_attn.inputs << block.attention.e_attn.dmu
                    block.attention.z_q.j << block.attention.E_attn.outputs
                    block.attention.z_k.j << block.attention.E_attn.outputs
                    block.attention.z_v.j << block.attention.E_attn.outputs




                    block.attention.E_qkv.inputs << summ(block.attention.e_q.dmu,block.attention.e_k.dmu,block.attention.e_v.dmu)
                    block.attention.z_qkv.j << block.attention.E_qkv.outputs
                    ...
                    if blocks == 0:
                        ...
                        block.attention.z_qkv.j_td << self.blocks[blocks-1].mlp.e_mlp.dtarget
                    
                    # my code here 
                    block.attention.z_q.j_td << block.attention.e_q.dtarget
                    block.attention.z_k.j_td << block.attention.e_v.dtarget
                    block.attention.z_v.j_td << block.attention.e_k.dtarget
                    ...
                    # 2 factor hebbian update 


                    block.attention.W_q.pre << block.attention.z_qkv.zF
                    block.attention.W_q.post << block.attention.e_q.dmu
                    
                    block.attention.W_k.pre << block.attention.z_qkv.zF
                    block.attention.W_k.post << block.attention.e_k.dmu
                    
                    block.attention.W_v.pre << block.attention.z_qkv.zF
                    block.attention.W_v.post << block.attention.e_v.dmu
                ...
                # self.embedding.W_embed.inputs << self.z_embed.zF
                ...
                for i in range(n_layers):
                    block = self.blocks[i]
                    advance_process >> block.attention.z_q.advance_state
                    advance_process >> block.attention.e_q.advance_state
                    advance_process >> block.attention.z_k.advance_state
                    advance_process >> block.attention.e_k.advance_state
                    advance_process >> block.attention.z_v.advance_state
                    ...
                    advance_process >> block.attention.z_q.reset
                    advance_process >> block.attention.e_q.reset
                    advance_process >> block.attention.z_k.reset
                    advance_process >> block.attention.e_k.reset
                    advance_process >> block.attention.z_v.reset
                    advance_process >> block.attention.e_v.reset
    ...
    def _dynamic(self, processes):
        ...
        for i in range(self.n_layers):
            var2 = self.circuit.get_components(
                f"block{i}_z_q",f"block{i}_z_k",f"block{i}_z_v",f"block{i}_e_q",f"block{i}_e_k",f"block{i}_e_v",f"block{i}_E_qkv",f"block{i}_W_q",f"block{i}_W_k",f"block{i}_W_v",
    ...
    def process(self, obs, lab, adapt_synapses=True):
        ...
        for i in range(self.n_layers):
            ...
            block.attention.E_qkv.weights.set(jnp.transpose(block.attention.W_q.weights.value))
        ...
        if adapt_synapses:
            ...
            for i in range(self.n_layers):
                block = self.blocks[i]
                # Print values before adding
                print("e_attn:", block.attention.e_attn.L.value)
                print("e_mlp:", block.mlp.e_mlp.L.value)
                print("e_mlp1:", block.mlp.e_mlp1.L.value)
                print("e_q:", block.attention.e_q.L.value)
                print("e_k:", block.attention.e_k.L.value)
                print("e_v:", block.attention.e_v.L.value)

                # Now add them to block_errors
                block_errors += (
                    block.attention.e_attn.L.value +
                    block.mlp.e_mlp.L.value +
                    block.mlp.e_mlp1.L.value +
                    block.attention.e_q.L.value +
                    block.attention.e_k.L.value +
                    block.attention.e_v.L.value
                )
